...
- **CF sudah dikonfigurasi dengan info koneksi RC** (21 Agustus 2026): nama/instance ID (`RC-8ae1f3f3-8542-48be-9c3d-ae51a2ed3ffd`)/shared secret RC sudah tersimpan di Settings & Script Properties CF lewat `updateRecordCenterConnection_` — lihat `record-center/README.md`. Endpoint URL sengaja masih kosong sampai RC-01 di-deploy; outbox tetap `PENDING_CONFIGURATION`, ini status valid.
- Langkah berikutnya: bangun RC-01 (endpoint federatif + `RC_INBOX_EVENTS`/`RC_DECISION_OUTBOX`/tabel penerimaan usul + event `QUERY_DECISION`), deploy sebagai Web App, masukkan URL-nya ke CF, baru lanjut UAT pengiriman `UP-2026-0002` sesuai urutan di handoff §1.3/§21.
